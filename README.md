...
# Symbolic Thinking
LLMs are not efficient at capturing facts in a retrievable fashion. Estimates range from 100-1000 parameters required per persistantly memorized fact. Another practical limit to model-based memory is trust. In his recent keynote at San Francisco Startup School Andrej Karpathy [describes LLM intelligence as ___jagged intelligence___](https://youtu.be/LCEmiRjPEtQ?si=nl50N-IIirOuKw3B&t=980). LLM generations are useful, but useres can not reliably verify if a fact has been retrieved from persistant memory, or rehydrated (read:hallucinated) from a generalization. 

Working memory should not be black boxed since the provenance of details used for reasoning and tasks need to remain traceable for synergistic human-centered collaboration. The alternative could be broawseable ___knowledge graphs___ with shards made up of entities and their prolific realtionships in a graph. The graph nodes and vertices are generated by generative shard defnitions comprised of verbatim quote / name, contextual definition and if applicable a 'canonical' abstraction supplied by the LLM.
...
This multi-resolution of terms in local context and global contexts reflects the model's understanding of our world, adding a rhizome of abstraction realtionships aligning the embeddings of our knwoledge shards with common sense-making patterns, conventions, laws, and practices. 

## Human centered AI flow

__loop:__
- locate, delimitate attention
- define issue
- identify, retrieve solution candidates from targeted and neighbouring knowledge fields
- customize and assemble 
- explore solution fitness against retrieved and ideated solution candidate components
- delimitate issue and addressable fixes
- iterate

- Hu: here is the issue
- AI: let me retrieve, augment, and summarize 
- 
## Attention Management
Complex agentic workflows require attention management to device strategies for navigating the knowledge graph both for framing a problem and for the explorative search of solution candidates in said graph.

If we allow the analog of attention to a rolling context window with references to relevant knowledge shards, we can design for agentic attention management. Agents would use hybrid search, distinct references, or strategic instructions to comprise the context for an adequate capture of a described problem. Each modality for navigating the graph needs to be callable by an attention planning agent. These include but are not limited to:
 
- `specific issue` > `symbolic issue` > `symbolic solution candidate` > `specific solution candidate`

The resulting problem formulation would feature a ranked list of issues as captured by exact matches in the database and augmented with the descriptions of the prompting user. To find a solution we devise a strategy for ranking solution component candidates by their disposition to solve the captured problem. In interaction with the user(s) the agent will navigate the specific, global, and symbolic graph layers and model knowledge to search the most promising constellation of solution. 
